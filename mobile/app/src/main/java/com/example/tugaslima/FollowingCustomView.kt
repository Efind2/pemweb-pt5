...
class FollowingCustomView(context: Context, attributeSet: AttributeSet) : View(context, attributeSet) {
    ...
    private val users = mutableListOf(
        User("BASE UINSA", "@uinsafess", "Kirim menfess kamu di @UINSAFESS!!", true),
        User("SBMPTNFESS", "@sbmptnfess", "Sharing about SNMPTN, SBMPTN, etc", true),
        User("BASE UINSA", "@uinsafess", "Kirim menfess kamu di @UINSAFESS!!", true),
        User("SBMPTNFESS", "@sbmptnfess", "Sharing about SNMPTN, SBMPTN, etc", true),
        User("BASE UINSA", "@uinsafess", "Kirim menfess kamu di @UINSAFESS!!", true),
        User("SBMPTNFESS", "@sbmptnfess", "Sharing about SNMPTN, SBMPTN, etc", true),
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (user in users) {
            ...
            paint.color = if (user.isFollowing) Color.LTGRAY else Color.RED
            paint.style = Paint.Style.STROKE
            paint.strokeWidth = 3f
            canvas.drawRoundRect(buttonRect, 50f, 50f, paint)

            paint.style = Paint.Style.FILL
            paint.color = if (user.isFollowing) Color.BLACK else Color.RED
            paint.textSize = 40f

            val buttonText = if (user.isFollowing) "Following" else "Follow"
    ...
    data class User(val name: String, val username: String, val bio: String, var isFollowing: Boolean)
}